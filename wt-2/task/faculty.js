...
app.get('/faculty/start/:start',(req,res)=>{
    const start=req.params.start;

    const index=faculties.filter(faculty=>faculty.FacultyName.toLowerCase().startsWith(start))

    const ans={
        isSuccess: true,
        count: index.length,
        msg:'Data reterived successfully',
        data: index
    }
...
app.get('/faculty/born/:born',(req,res)=>{
  const born=req.params.born;

  const index=faculties.filter(faculty=>faculty.FacultyDateOfBirth.split('-')[0]<born)
